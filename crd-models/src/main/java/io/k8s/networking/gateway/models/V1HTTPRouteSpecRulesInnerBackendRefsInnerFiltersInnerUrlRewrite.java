...
public class V1HTTPRouteSpecRulesInnerBackendRefsInnerFiltersInnerUrlRewrite {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1HTTPRouteSpecRulesInnerBackendRefsInnerFiltersInnerUrlRewrite
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1HTTPRouteSpecRulesInnerBackendRefsInnerFiltersInnerUrlRewrite.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1HTTPRouteSpecRulesInnerBackendRefsInnerFiltersInnerUrlRewrite.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1HTTPRouteSpecRulesInnerBackendRefsInnerFiltersInnerUrlRewrite` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("path") != null && !jsonObj.get("path").isJsonNull()) {
        V1HTTPRouteSpecRulesInnerBackendRefsInnerFiltersInnerUrlRewritePath.validateJsonObject(jsonObj.getAsJsonObject("path"));
      }
  }
