...
public class V1HTTPRouteSpecParentRefsInner {
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1HTTPRouteSpecParentRefsInner
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1HTTPRouteSpecParentRefsInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1HTTPRouteSpecParentRefsInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1HTTPRouteSpecParentRefsInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
