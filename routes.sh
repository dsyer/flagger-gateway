...
function update() {
	...
	curl -H "Content-Type: application/json" localhost:8001/actuator/gateway/routes/$name -d "$body"
	if [ $? -eq 0 ]; then
...
while curl -s -X POST localhost:8001/actuator/gateway/refresh; do
	date
	. <(curl -s localhost:8001/actuator/gateway/routes | jq -r '.[] | "curl -s -X DELETE localhost:8080/actuator/gateway/routes/\(.route_id)"')
